...
async def connect(args, setup=True):
    ...
    try:
        ...
        if 'BENCH_HCI_PACKET_OVERRIDE' in os.environ:
            ...
            if args.adapter not in ('AC:A7:F1:83:27:AD', 'usb:2357:0604') or override not in (251, 502):
                raise ValueError('packet-size experiment is restricted to the owned UB500 and 251/502 bytes')
            if queue.pending:
