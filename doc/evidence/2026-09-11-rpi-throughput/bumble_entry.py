...
from bumble.device import Device, ConnectionParametersPreferences
from bumble_bleak import _backend
from bumble_bleak import BleakScanner

if 'BENCH_INITIAL_MS' in os.environ:
    initial_ms = float(os.environ['BENCH_INITIAL_MS'])
    ConnectionParametersPreferences.default = ConnectionParametersPreferences(
        connection_interval_min=initial_ms, connection_interval_max=initial_ms,
        supervision_timeout=4000)
    print('BUMBLE initial_interval_ms=%s initial_timeout_ms=4000' % initial_ms, flush=True)
...
async def connect(args, setup=True):
    ...
    try:
        ...
        advertised_packet_size = queue.max_packet_size
        if 'BENCH_HCI_PACKET_OVERRIDE' in os.environ:
            override = int(os.environ['BENCH_HCI_PACKET_OVERRIDE'])
            if args.adapter not in ('AC:A7:F1:83:27:AD', 'usb:2357:0604') or override != 251:
                raise ValueError('only the owned UB500 / 251-byte experiment is allowed; 502 corrupted a RAM payload')
            if queue.pending:
                await asyncio.wait_for(queue.drain(conn.handle), 5)
            if queue.pending:
                raise RuntimeError('HCI queue did not drain before the experiment')
            queue.max_packet_size = override
            print('EXPERIMENT NONSTANDARD HCI packet bytes initial_host_queue=%s override=%s credits_unchanged=%s'
                  % (advertised_packet_size, override, queue.max_in_flight), flush=True)
        ...
        current_buffers = await conn.device.host.send_sync_command(HCI_LE_Read_Buffer_Size_Command())
        print('BUMBLE LIVE LE BUFFER packet_bytes=%s credits=%s host_queue_bytes=%s'
              % (current_buffers.le_acl_data_packet_length,
                 current_buffers.total_num_le_acl_data_packets, queue.max_packet_size), flush=True)
        if os.environ.get('BENCH_REFRESH_HCI') == '1':
            packet_bytes = current_buffers.le_acl_data_packet_length
            packet_credits = current_buffers.total_num_le_acl_data_packets
            if not 27 <= packet_bytes <= 1021 or packet_credits != queue.max_in_flight:
                raise RuntimeError('live HCI buffer result needs separate validation')
            if queue.pending:
                await asyncio.wait_for(queue.drain(conn.handle), 5)
            if queue.pending:
                raise RuntimeError('HCI queue did not drain before refreshing size')
            queue.max_packet_size = packet_bytes
            print('BUMBLE host packet size refreshed from actual HCI response', flush=True)
        diagnostic = dict(transport=link._cli._backend.adapter,
                          hci_initial_host_packet_bytes=advertised_packet_size,
                          hci_reported_packet_bytes=current_buffers.le_acl_data_packet_length,
        ...
        # Bound host buffering independently of controller HCI credits. The
        # explicit experiment above may override packet size; credits stay
        # at the controller's advertised count.
