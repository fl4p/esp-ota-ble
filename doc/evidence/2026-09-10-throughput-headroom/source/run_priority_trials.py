...
def trial(name,chunk):
    ...
    with path.open('w') as f:
        proc=subprocess.run([PY,str(ROOT/'run_native_priority.py'),'Graceful',name,'--chunk',str(chunk)],
    ...
    row=rows[-1]
    print(name,{k:row.get(k) for k in ('kB_s','elapsed','verified','real_write','thread_qos_is_userInteractive')},flush=True)
    install(name+'-restore','Graceful')
    return row['kB_s']

rates=[]
for chunk in (244,495):rates.append((trial('priority-chunk'+str(chunk),chunk),chunk))
best,chunk=max(rates)
repeats=[]
if best>88.282:
    for i in range(1,4):repeats.append(trial('priority-repeat-'+str(i),chunk))
...
(ROOT/'priority-summary.json').write_text(json.dumps(summary,indent=2)+'\n')
