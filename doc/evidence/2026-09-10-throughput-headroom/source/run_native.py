...
ap.add_argument('--image',default='/tmp/node_X.bin');ap.add_argument('--coc',action='store_true');args=ap.parse_args()
...
with path.open('w') as f:
    proc=subprocess.run([str(R/'native_push'),args.image,ids[args.label]['image_sha'],*(['--coc'] if args.coc else [])],
...
row=dict(native,image=args.image,after=after,verified=verified,real_write=real,
         ...
         skip=fields('OTAB SKIP '),wire='raw',wire_bytes=len(image),chunk=None if args.coc else 495,
         stream_write_max=4096 if args.coc else None,burst=1,
         ...
         purpose='measure',source_sha256={name:hashlib.sha256((R/name).read_bytes()).hexdigest()
            for name in ('native_push.swift','run_native.py','node/tools/bench_ble_ota.py')})
