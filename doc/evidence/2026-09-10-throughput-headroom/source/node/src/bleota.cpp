...
#ifdef PROTO_BLE_COC_BENCH
#ifndef PROTO_BLE_COC_MTU
#define PROTO_BLE_COC_MTU 2048
#endif
// Private alternative data bearer. Control/auth/credits and the flash verifier
// remain the existing OTA protocol. A CoC frame is only another stream fragment.
class BenchCocCallbacks : public NimBLEL2CAPChannelCallbacks {
    bool shouldAcceptConnection(NimBLEL2CAPChannel*) override {
        return have_conn() && authed() && !otaBleActive();
    }
    void onConnect(NimBLEL2CAPChannel* channel,uint16_t mtu) override {
        if (channel->getConnHandle()!=conn_handle || !authed()) {
            channel->disconnect();return;
        }
        char line[100];snprintf(line,sizeof(line),"BENCH COC min_mtu=%u local_mtu=%u local_mps=%u",
            (unsigned)mtu,(unsigned)PROTO_BLE_COC_MTU,(unsigned)MYNEWT_VAL(BLE_L2CAP_COC_MPS));
        tx_append_line(line);
    }
    void onRead(NimBLEL2CAPChannel* channel,std::vector<uint8_t>& data) override {
        if (channel->getConnHandle()!=conn_handle || !authed() || !otaBleActive()) return;
        otaBleStageBytes(data.data(),data.size());
    }
    void onDisconnect(NimBLEL2CAPChannel* channel) override {
        if (otaBleActive()) otaBleRequestAbort();
    }
};
#endif
...
static void ota_restart_hook(void)
{
    ...
    if (!serial_driving.load(std::memory_order_acquire)) {
#ifdef PROTO_BLE_BENCH_GRACEFUL_REBOOT
        tx_append_line("BENCH REBOOT graceful=1");
#endif
        drain_tx();
        delay(50);
#ifdef PROTO_BLE_BENCH_GRACEFUL_REBOOT
        // OTA end has already validated and selected the new boot partition.
        // Send LL termination so the central need not wait for supervision
        // timeout after the radio vanishes. Bound the wait; reboot still wins
        // if termination cannot be delivered. The host verifies the new boot.
        if (have_conn()) {
            auto *server=NimBLEDevice::getServer();
            if (server) server->disconnect(conn_handle);
            for (unsigned n=0;n<20 && have_conn();++n) delay(5);
        }
#endif
    }
...
class CtrlCb : public NimBLECharacteristicCallbacks {
    void onWrite(NimBLECharacteristic *c, NimBLEConnInfo &info) override {
...
#if PROTO_BLE_BENCH
        if (v == "flashinfo") {
            if (otaBleActive()) { tx_append_line("BENCH FLASH unavailable while OTA active"); return; }
            auto *chip=esp_flash_default_chip;
            if (!chip || !chip->chip_drv) { tx_append_line("BENCH FLASH unknown"); return; }
            char line[140];
            auto caps=chip->chip_drv->get_chip_caps ? chip->chip_drv->get_chip_caps(chip) : 0;
            snprintf(line,sizeof(line),"BENCH FLASH id=%06lx driver=%s bytes=%lu caps=%lu",
                (unsigned long)chip->chip_id,chip->chip_drv->name,
                (unsigned long)chip->size,(unsigned long)caps);
        ...
        if (v == "buffers") {
            char line[100];
            const esp_bt_controller_config_t cfg=BT_CONTROLLER_INIT_CONFIG_DEFAULT();
            snprintf(line,sizeof(line),"BENCH BUFFERS msys1=%u size=%u msys2=%u acl=%u ce=%u",
                ...
                (unsigned)cfg.ce_len_type);
            tx_append_line(line); return;
        }
        if (v == "nosleep") {
            const esp_err_t rc=otaBleActive() ? ESP_ERR_INVALID_STATE : esp_bt_sleep_disable();
            char line[80];snprintf(line,sizeof(line),"BENCH NOSLEEP rc=%d",(int)rc);
            tx_append_line(line);return;
        }
        if (v == "rssi") {
            int8_t rssi=0;const int rc=ble_gap_conn_rssi(conn_handle,&rssi);
            char line[80];snprintf(line,sizeof(line),"BENCH RSSI rc=%d dbm=%d",rc,(int)rssi);
            tx_append_line(line);return;
        }
        if (v == "dleinfo") {
            char line[100];
        ...
        if (v == "link") {
            ...
            snprintf(line, sizeof(line), "BENCH LINK interval=%u mtu=%u phy_known=%u tx=%u rx=%u timeout=%u latency=%u",
                     (unsigned)info.getConnInterval(), (unsigned)info.getMTU(),
                     (unsigned)known, (unsigned)tx, (unsigned)rx,
                     (unsigned)info.getConnTimeout(),(unsigned)info.getConnLatency());
            tx_append_line(line);
            return;
        }
        if (v.compare(0,8,"timeout ")==0) {
            char *end=nullptr; const char *p=v.c_str()+8;
            unsigned long n=strtoul(p,&end,10);
            if (otaBleActive() || end==p || *end || n<10 || n>3200 ||
                n*8<=info.getConnInterval()*2UL*(info.getConnLatency()+1)) {
                tx_append_line("BENCH timeout invalid or OTA active"); return;
            }
            srv->updateConnParams(conn_handle,info.getConnInterval(),info.getConnInterval(),0,n);
            tx_append_line("BENCH timeout requested; inspect link after settling");return;
        }
...
#ifdef PROTO_BLE_COC_BENCH
    auto coc=NimBLEDevice::createL2CAPServer();
    if (!coc || !coc->createService(150,PROTO_BLE_COC_MTU,new BenchCocCallbacks())) {
        Serial.println("!! bench CoC initialization failed");
        return;
    }
#endif
