...
final class Push: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate, StreamDelegate {
    ...
    init(path: String, base: String, transform: String, outputBytes: Int) throws {
        let useCoc=false
        self.useCoc=useCoc
        self.transform=transform;self.outputBytes=outputBytes
        ...
        super.init()
        guard bytes.count > 0, outputBytes > 0, ["tamp","delta"].contains(transform), expectedBase.count == 64 else {
    ...
    func begin() {
        guard stage == "info", before["base"] as? String == expectedBase,
              before["run"] != nil, let slot=before["slot"] as? Int, slot>=outputBytes else {
        ...
        command("begin \(bytes.count) \(digest) \(transform) \(outputBytes)")
    }
    func pump() {
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral p: CBPeripheral,error: Error?) {
        ...
        let record:[String:Any] = ["elapsed":elapsed(),"bytes":bytes.count,"before":before,
            ...
            "wire":transform,"image_bytes":outputBytes,
...
guard CommandLine.arguments.count==5, let outBytes=Int(CommandLine.arguments[4]) else {
    print("usage: native_push_xform PAYLOAD BASE tamp|delta IMAGE_BYTES");exit(2)
}
do {
    let push=try Push(path:CommandLine.arguments[1],base:CommandLine.arguments[2],
        transform:CommandLine.arguments[3],outputBytes:outBytes)
