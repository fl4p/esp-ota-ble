...
final class Push: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate, StreamDelegate {
    ...
    init(path: String, base: String, useCoc: Bool) throws {
        self.useCoc=useCoc
...
guard CommandLine.arguments.count==3 || (CommandLine.arguments.count==4 && CommandLine.arguments[3]=="--coc") else {
    print("usage: native_push IMAGE EXPECTED_RECEIVER_IMAGE_SHA [--coc]");exit(2)
}
do {
    let push=try Push(path:CommandLine.arguments[1],base:CommandLine.arguments[2],useCoc:CommandLine.arguments.count==4)
