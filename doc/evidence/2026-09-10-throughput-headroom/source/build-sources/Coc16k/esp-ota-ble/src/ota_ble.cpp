...
#endif
#ifndef OTA_BLE_BENCH_BUFFER_BYTES
#define OTA_BLE_BENCH_BUFFER_BYTES 4096
#endif
static_assert(OTA_BLE_BENCH_BUFFER_BYTES >= 256 && OTA_BLE_BENCH_BUFFER_BYTES <= 16384 &&
              OTA_BLE_BENCH_BUFFER_BYTES % 256 == 0, "bench buffer must hold whole flash pages");
...
#if OTA_BLE_BENCH_STRATEGY == 2 || OTA_BLE_BENCH_STRATEGY == 4 || OTA_BLE_BENCH_STRATEGY == 5 || (defined(CONFIG_SPI_FLASH_YIELD_DURING_ERASE) && !defined(CONFIG_ESP_TASK_WDT_PANIC))
...
static bool benchBufferedBlocks = false;
...
static esp_err_t commitSector() {
    ...
    secOff += n;
    secFill = 0;
    if (benchBufferedBlocks) {
        esp_err_t err=eraseAhead(off+n+ERASE_LEAD);
        if(err!=ESP_OK)return err;
        const int64_t started=esp_timer_get_time();
        err=esp_ota_write_with_offset(otaHandle,secBuf,n,off);
        benchProgramUs+=esp_timer_get_time()-started;
        if(err==ESP_OK){
            benchProgramBytes+=n;
            ++skWritten;
...
static void grantCredit() {
    ...
    if (
#if OTA_BLE_BENCH_DEDUP_CREDIT
        g > lastGranted &&
#endif
        (g >= lastGranted + credStep() || g == expectedSize)) {
        lastGranted = g;
...
#elif OTA_BLE_BENCH_STRATEGY == 2 || OTA_BLE_BENCH_STRATEGY == 4 || OTA_BLE_BENCH_STRATEGY == 5
    {
...
#if OTA_BLE_BENCH_STRATEGY == 4 || OTA_BLE_BENCH_STRATEGY == 5
            // Batch program calls; erase coverage remains independently block-sized.
            secBuf=(uint8_t*)heap_caps_malloc(OTA_BLE_BENCH_BUFFER_BYTES,
#if OTA_BLE_BENCH_STRATEGY == 5
                MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT
#else
                MALLOC_CAP_DEFAULT
#endif
            );
            if(!secBuf)err=ESP_ERR_NO_MEM;
            else {
                secSize=OTA_BLE_BENCH_BUFFER_BYTES;
                secOff=secFill=0;
                benchBufferedBlocks=true;
                benchStrategy=OTA_BLE_BENCH_STRATEGY==5 ? "internal" : "buffered";
            }
#endif
        }
    }
...
bool otaBleEnd() {
    ...
    // Keep each diagnostic below emit()'s 96-byte buffer even at large counts.
    emit(OtaBleLevel::Info, "OTAB FLASH mode=%s program_bytes=%u erase_bytes=%u",
         benchStrategy, (unsigned) benchProgramBytes, (unsigned) benchEraseBytes);
    emit(OtaBleLevel::Info, "OTAB TIME begin_ms=%u program_ms=%u compare_us=%u",
         (unsigned) (benchBeginUs / 1000), (unsigned) (benchProgramUs / 1000), (unsigned) benchCompareUs);
